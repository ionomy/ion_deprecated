...
static void NotifyionNodeUpdated(MasternodeManager *page, CionNodeConfig nodeConfig)
{
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyionNodeChanged.connect(boost::bind(&NotifyionNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyionNodeChanged.disconnect(boost::bind(&NotifyionNodeUpdated, this, _1));
}
...
void MasternodeManager::updateionNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_ion);
    bool bFound = false;
    int nodeRow = 0;
...
{
    AddEditionNode* aenode = new AddEditionNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CionNodeConfig c = pwalletMain->mapMyionNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    ionNodeConfigDialog* d = new ionNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    confirm = QMessageBox::question(this, "Delete ion Node?", "Are you sure you want to delete this ion node configuration?", QMessageBox::Yes|QMessageBox::No);
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyionNodes.erase(sAddress);
        walletdb.EraseionNodeConfig(c.sAddress);
...
void MasternodeManager::on_startButton_clicked()
{
    ...
    QMessageBox msg;
    if(result)
        msg.setText("ion Node at " + QString::fromStdString(c.sAddress) + " started.");
    else
...
void MasternodeManager::on_stopButton_clicked()
{
    ...
    if(result)
    {
        msg.setText("ion Node at " + QString::fromStdString(c.sAddress) + " stopped.");
    }
    else
    {
        msg.setText("Error: " + QString::fromStdString(errorMessage));
