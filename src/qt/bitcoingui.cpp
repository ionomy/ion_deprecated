...
#endif
    setObjectName("ion");
    setStyleSheet("#ion { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #efefef, stop: 1 #dedede);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
    ...
    else
    {
        ...
        if(count != prevBlocks)
        {
            labelBlocksIcon->setPixmap(QIcon(QString(
                fUseBlackTheme ? ":/movies/spinner-dark-%1" : ":/movies/spinner-%1").arg(spinnerFrame, 3, 10, QChar('0')))
